...
A report of the pipeline and final source tables can be generated using build_report 
after finishing the full pipeline.
Once this has been run, you can merge motif sequences from different pipeline 
runs by using the Merge_motifs_and_report.Rmd file.
It gerenates a general list of highstab and lowstab motifs. 
Then different python scripts exist out of the pipeline to get list of wanted
...
1. High and low stability UTR locations (inputs to STREME and HOMER)
...
2. backgroud.bed : bed file consisting of all 3'UTR sequences of transcript
   detected by the pipeline_slamdunk_umis (and extra input to HOMER).
3. fire
...
* Software:
    - python (tested on v3.8.12 with pysam v0.17.0 when built)
...
The report render ouputs in final_motifs pipeline_report.html and 
[highstab|lowstab]_motifs_source.tsv tables.
...
# We want to only run this if fasta files arn't provided. `from_fasta` will be true
# if fasta files *aren't* provided.
...
def extractFire(infile, outfiles):
    ...
    filtering so that at least 25% of the input sequences had that motif'''
...
def removeRedundantFire(infiles, outfile):
    '''Remove redundants motifs from fire output using multiple sequence alignment.
    The aim here is for the downstream analysis of FIRE to be as similar as that
    for STREME and HOMER, this fulfills the tomtom self comparison step'''
